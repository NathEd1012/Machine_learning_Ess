...
def state_to_features(game_state: dict) -> np.array:
    ...
    #print(next_move_enemy_features)
    #layout(game_state)
    ...
    for enemy_pos in targets:
        ...
        while queue:
            ...
            if abs(x - enemy_x) + abs(y - enemy_y) == 1:
                # Can't move into enemy tile, record adjacent tile
            ...
            for i, (dx, dy) in enumerate(directions):
                ...
                if 0 <= new_x < rows and 0 <= new_y < cols and (new_x, new_y) not in visited:
                    if (field[new_x, new_y] == 0 and 
                        danger_map[new_x, new_y] >= -1 / 2 and
                        (new_x, new_y) not in targets): # Free tile, no danger, no enemy
...
def is_tile_safe_at_time(x, y, time, bombs, field):

    for bomb_pos, bomb_timer in bombs:
        bx, by = bomb_pos
        explosion_time = bomb_timer

        # Check if bomb explodes when we arrive or after
        if time <= explosion_time + 3:
            # Check if (x, y) in explosion range
            if is_in_bomb_range(x, y, bx, by, field):
                # If tile in range when bomb explodes
                if time >= explosion_time and time <= explosion_time + 3:
                    return False
                
    return True


def is_in_bomb_range(x, y, bx, by, field):

    if x == bx and abs(y - by) <= 3:

        for i in range(1, abs(y - by) + 1):
            step_y = by + i * ((y - by) // abs(y - by))
            if field[bx, step_y] == -1:
                return False
            
        return True
    
    elif y == by and abs(x - bx) <= 3:

        for i in range(1, abs(x - by)) <= 3:
            step_x = bx + i * ((x - bx) // abs(x - bx))

            if field[step_x, by] == -1:
                return False
        return True
    return False
...
def get_path_bfs_safe_tile(game_state):
    ...
    queue = deque([(start_x, start_y, 0, None)])  
    visited[(start_x, start_y)] = 0

    # BFS to find shortest path to a safe tile (free and outside danger)
    while queue:
        x, y, time, first_move = queue.popleft()

        if is_tile_safe_at_time(x, y, time, bombs, field):
            if is_tile_safe_at_time(x, y, time + 1, bombs, field):
                if first_move is not None:
                    return [first_move]
                else:
                    return[-1]
        ...
        for i, (dx, dy) in enumerate(directions):
            ...
            if 0 <= new_x < rows and 0 <= new_y < cols and field[new_x, new_y] == 0:
                if (new_x, new_y) in enemies:
                    continue

                if ((new_x, new_y) not in visited) or (visited[(new_x, new_y)] > new_time):
                    if is_tile_safe_at_time(new_x, new_y, new_time, bombs, field):
                        visited[(new_x, new_y)] = new_time

                        if first_move is None:
                            queue.append((new_x, new_y, new_time, direction_names[i]))
                        else:
                            queue.append((new_x, new_y, new_time, first_move))
