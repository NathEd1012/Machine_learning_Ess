...
from .features import (state_to_features, 
                       ...
                       get_danger_map, 
                       is_useless_bomb, 
                       calculate_crates_destroyed, 
                       calculate_enemies_in_blast_radius, 
                       is_enemy_in_dead_end,
                       get_path_bfs_safe_tile)
import numpy as np
...
# Data log every LOG_FREQUENCY rounds
LOG_FREQUENCY = 10
...
def custom_events(self, old_game_state, self_action, new_game_state, events):
    ...
    # Give rewards for enemies potentially destroyed
    if self_action == "BOMB" and old_game_state['self'][2] == True:
        enemies_in_blast_radius = calculate_enemies_in_blast_radius(new_game_state)
        if enemies_in_blast_radius > 0:
    ...
    new_enemies = len(new_game_state['others'])
    if old_enemies > 0 and new_enemies == 0:
    ...
    # Check if enemy in dead end
    if new_enemies:
        #agent_x, agent_y = own_position
        nearest_enemy_new = nearest_enemy(new_game_state)

        enemy_in_dead_end = is_enemy_in_dead_end(new_game_state, nearest_enemy_new, threshold = 1)
