...
def state_to_features(game_state: dict) -> np.array:
    ...
    # Bomb_cooldown
    bomb_cooldown = get_bomb_cooldown(game_state)
    ...
    features = np.concatenate([
            ...
            bomb_cooldown # 1: bomb cooldown
    ])
    return features

def get_bomb_cooldown(game_state):
...
def get_action_to_nearest(game_state, target):
    position = game_state['self'][-1]
    steps = [(0, -1), (1, 0), (0, 1), (-1, 0)]
    step_names = [0, 1, 2, 3] # up, right, down, left

    if target == 'coins':
        target = game_state['coins']
    if target == 'crates':
        field = game_state['field']
        target = [(x,y) for x in range(1,field.shape[0]-1) for y in range(1,field.shape[1]-1)
                  if (field[x,y]==1)]
    
    nearest_target_pos = [min(list1, key=lambda p: manhattan_dist(position, p))]
    nearest_target_dist = min([manhattan_dist(position,p) for p in target])

    if nearest_target_dist == 1:
        for step in steps:
'''
