...
def state_to_features(game_state: dict) -> np.array:
    ...
    features = np.concatenate([
            ...
            next_move_enemy_features # 2: next move to target
    ])
...
def get_path_bfs_enemies(game_state):
    ...
    # BFS to find shortest path
    while queue:
        x, y, first_move, distance = queue.popleft()
        ...
        if (x, y) in targets:
            ...
            if first_move is not None:
                #print(first_move, direction_names)
                return [direction_names[first_move]]#, distance]
        ...
        for i, (dx, dy) in enumerate(directions):
            ...
            if 0 <= new_x < rows and 0 <= new_y < cols and (new_x, new_y) not in visited:
                if (field[new_x, new_y] == 0 and 
                    ...
                    # Enque new position, passing first move
                    if first_move is None:
                        queue.append((new_x, new_y, direction_names[i], distance + 1))
                    else:
                        queue.append((new_x, new_y, first_move, distance + 1))

    # Return if no path to target
    return [-1]#, float('-inf')] # No valid move
