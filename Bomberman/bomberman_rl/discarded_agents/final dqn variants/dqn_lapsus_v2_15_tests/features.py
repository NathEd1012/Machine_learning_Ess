...
def state_to_features(game_state: dict) -> np.array:
    ...
    #layout(game_state)
    l1_surroundings_features = get_local_vision_grid(game_state)

    
    features = np.concatenate([
            #neighboring_tiles_features, # 4: up, right, down, left
            #bomb_features, # 5: up, right, down, left, here
            l1_surroundings_features, # 25 L1 <= 3 norm around my agent.
            ...
            #enemy_safe_tiles_features # 1: How many safe tiles do my enemies have if I place a bomb here?
    ])
    ...
    for dx in range(-3, 4):
        for dy in range(-3, 4):
            ...
            if x < 0 or x >= rows or y < 0 or y >= cols:
                ...
                # Tile Type
                if field[x, y] == -1:
                    encoded_value |= 1  # Wall
                elif field[x, y] == 1:
                    encoded_value |= 2  # Crate

                # Bombs
                if (x, y) in bomb_positions:
                    encoded_value |= 4  # Bomb present

                # Enemies
                if (x, y) in enemies:
                    encoded_value |= 8  # Enemy present

                # Explosions
                if explosion_map[x, y] > 0:
                    encoded_value |= 16  # Explosion occurring

                # Coins
                if (x, y) in coins:
                    encoded_value |= 32  # Coin present

            # Append encoded_value to vision_features
            vision_features.append(encoded_value)

    return np.array(vision_features) / 64
...
def get_neighboring_tiles(own_position, game_state):
    ...
    # Up x, y - 1
    if y - 1 >= 0:
        tile_free = field[x, y - 1] == 0
        no_enemy = (x, y - 1) not in enemy_positions
        no_explosion = explosion_map[x, y - 1] == 0
        tile_up = 1 if tile_free and no_enemy and no_explosion else 0
    else: 
        tile_up = 0

    # Right x + 1, y
    if x + 1 < cols:
        tile_free = field[x + 1, y] == 0
        no_enemy = (x + 1, y) not in enemy_positions
        no_explosion = explosion_map[x + 1, y] == 0
        tile_right = 1 if tile_free and no_enemy and no_explosion else 0
    else:
        tile_right = 0
    ...
    if y + 1 < rows:
        ...
        no_enemy = (x, y + 1) not in enemy_positions
        no_explosion = explosion_map[x, y + 1] == 0
        tile_down = 1 if tile_free and no_enemy and no_explosion else 0
    else:
        tile_down = 0
    ...
    if x - 1 >= 0:
        ...
        no_enemy = (x - 1, y) not in enemy_positions
        no_explosion = explosion_map[x - 1, y] == 0
        tile_left = 1 if tile_free and no_enemy and no_explosion else 0
    else:
        tile_left = 0
